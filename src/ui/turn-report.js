...
export function showTurnReport(state) {
  ...
  for (const f of aiFactions) {
    ...
    aiReport += `<div style="font-size:12px;color:#6B5B4F;margin:2px 0">${f.name}：${aiCities.length} 城 · ${armies.length} 支部队</div>`;
  }
  ...
  const victoryMsg = victoryProgress >= 100
    ? '<div style="color:#C43A30;font-size:18px;font-weight:bold;text-align:center;margin:8px 0;font-family:\'KaiTi\',\'STKaiti\',serif">🎉 天下统一！你赢了！</div>'
    : `<div style="font-size:13px;color:#6B5B4F;text-align:center;margin:6px 0">统一进度：${playerCities.length}/${allCities.length} 城（${victoryProgress}%）</div>`;

  panel.innerHTML = `
    <div style="font-size:20px;color:#C43A30;font-weight:bold;border-bottom:1px solid #D4C5A0;padding-bottom:10px;margin-bottom:10px;font-family:'KaiTi','STKaiti',serif">
      📋 第 ${state.turn} 回合报告
    </div>
    ...
    <div style="margin-top:14px;text-align:right;border-top:1px solid #D4C5A0;padding-top:10px">
      <button id="btn-report-close" style="padding:8px 28px;background:#C43A30;color:#F7F2E8;border:1px solid #A83227;border-radius:4px;cursor:pointer;font-size:15px;font-family:'KaiTi','STKaiti',serif">继续</button>
    </div>
  `;
