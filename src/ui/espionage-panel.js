...
function renderSpySelect() {
  ...
  panel.innerHTML = `
    <div style="font-size:20px;color:#ccaa44;font-weight:bold;border-bottom:1px solid #665522;padding-bottom:10px;margin-bottom:12px">
      🕵️ 间谍行动
    </div>
    <div style="font-size:13px;color:#888;margin-bottom:10px">选择执行任务的武将（智力越高成功率越大）：</div>
    <div style="display:flex;flex-direction:column;gap:6px">
      ${generals.map(g => `
        <div class="spy-card" data-gid="${g.id}" style="display:flex;align-items:center;justify-content:space-between;
          background:#1a1a30;border:1px solid #333355;border-radius:5px;padding:10px 14px;cursor:pointer"
          onmouseenter="this.style.borderColor='#665522'" onmouseleave="this.style.borderColor='#333355'">
          <div>
            <span style="color:#ccaa44;font-weight:bold">${g.name}</span>
            <span style="font-size:12px;color:#888;margin-left:8px">智力${g.intelligence} 魅力${g.charisma}</span>
          </div>
          <span style="font-size:12px;color:#888">${g.inCity ? '在' + (_state.cities[g.inCity]?.name || g.inCity) : '空闲'}</span>
        </div>
      `).join('')}
    </div>
    <div style="margin-top:12px;text-align:right">
      <button id="btn-spy-close" style="padding:6px 22px;background:#443322;color:#ccaa44;border:1px solid #665522;border-radius:4px;cursor:pointer;font-size:14px;font-family:inherit">关闭</button>
    </div>
  `;
...
function renderActionSelect(spy) {
  ...
  panel.innerHTML = `
    ...
    <div style="font-size:13px;color:#888;margin-bottom:8px">选择行动和目标：</div>
    ...
    <div style="font-size:13px;color:#ccaa44;margin-bottom:6px">选择目标城池：</div>
    <div style="display:flex;flex-direction:column;gap:4px;max-height:180px;overflow-y:auto;margin-bottom:10px">
      ${cities.map(c => `
        <button class="btn-spy-target" data-cid="${c.id}" data-act="" style="text-align:left;
          padding:6px 12px;background:#1a1a30;border:1px solid #333355;border-radius:4px;cursor:pointer;font-family:inherit;font-size:13px;color:#ddd"
          onmouseenter="this.style.borderColor='#665522'" onmouseleave="this.style.borderColor='#333355'">
          ${c.name} <span style="color:#888;font-size:11px">${_state.factions[c.owner]?.name || '无主'} · 城防${c.defense} · 民心${c.stability}</span>
        </button>
      `).join('')}
    </div>
    <div style="margin-top:10px;text-align:right">
      <button class="btn-back-spy" style="padding:6px 14px;background:#333;color:#aaa;border:1px solid #555;border-radius:4px;cursor:pointer;font-size:13px;font-family:inherit;margin-right:8px">← 换人</button>
      <button id="btn-spy-close2" style="padding:6px 22px;background:#443322;color:#ccaa44;border:1px solid #665522;border-radius:4px;cursor:pointer;font-size:14px;font-family:inherit">关闭</button>
    </div>
  `;
  ...
  panel.querySelectorAll('[data-act]').forEach(btn => {
    btn.onclick = () => {
      ...
      panel.querySelectorAll('[data-act]').forEach(b => b.style.borderColor = '#333355');
      btn.style.borderColor = '#ccaa44';
    };
  });
...
function renderAction(act, label, desc, cost) {
  return `
    <div class="spy-action" data-act="${act}" style="display:flex;align-items:center;justify-content:space-between;
      background:#1a1a30;border:1px solid ${act === 'scout' ? '#665522' : '#333355'};border-radius:5px;padding:10px 14px;cursor:pointer">
      <div>
        <span style="color:#ccaa44;font-weight:bold">${label}</span>
        <span style="font-size:11px;color:#888;margin-left:8px">${desc}</span>
      </div>
      <span style="font-size:12px;color:#888">💰${cost}金</span>
    </div>`;
}
